"""
CNN script for v4 of the lensing estimation testing
- like v3 but with no kappa exaggeration.
"""
...
def test_avg(model):
    """
    Test trained model on unseen test set - averaging over estimates and comparing to the single truth.
    """
    ...
    # Plot
    _, ax = plt.subplots(ncols=2, figsize=2*plt.figaspect(0.5), sharex=True, sharey=True)
    ...
    ax[0].imshow(np.squeeze(test_prediction), **imshow_args)
    ax[1].imshow(np.squeeze(test_y)[0], **imshow_args)

    # # High contrast
    # imshow_args = {'extent': [0, lx_deg, 0, lx_deg], 'interpolation': 'none'}
    # ax[0].imshow(np.squeeze(test_prediction), **imshow_args)
    # ax[1].imshow(np.squeeze(test_y)[0], vmin=0, vmax=1, **imshow_args)
...
def test(model):
    ...
    # Load test data
    test_path = 'xyz/v4/test/test_3.npz'
    x_arr = 'lensed'
    y_arr = 'kappa'
    test_x, test_y, _ = prepare_data_from_npz(test_path, x_arr, y_arr, val_frac=0, scale=1, shuffle=False,
                                              augment=False)
...
def train_model(model, train_x, train_y, val_xy, epochs, checkpoint_path):
    ...
    class BatchLossLogger(keras.callbacks.Callback):
        """Custom callback to print out training loss per batch."""
        def on_train_batch_end(self, batch, logs=None):
            print(f'Batch {batch}: training loss = {logs["loss"]:.6f} ({time.strftime("%c")})')
...
def loop_models():
    ...
    # Training parameters
    epochs = 70
    checkpoint_dir = f'xyz/{epochs}epoch_10k/'
    ...
    models = [model_baseline, model_simple, model_complex]
    ...
    model_names = ['baseline', 'simple', 'complex']
    for model, model_name in zip(models, model_names):
        print(f'\nStarting model {model_name} at {time.strftime("%c")}\n')
        model.summary()
